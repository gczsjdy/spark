...
public class VectorizedPlainValuesReader extends ValuesReader implements VectorizedValuesReader {
  ...
  @Override
  public final void readBooleans(int total, ColumnVectorBase c, int rowId) {
  ...
  @Override
  public final void readIntegers(int total, ColumnVectorBase c, int rowId) {
    c.putIntsLittleEndian(rowId, total, buffer, offset - Platform.BYTE_ARRAY_OFFSET);
    offset += 4 * total;
  }

  @Override
  public final void readLongs(int total, ColumnVectorBase c, int rowId) {
    c.putLongsLittleEndian(rowId, total, buffer, offset - Platform.BYTE_ARRAY_OFFSET);
    offset += 8 * total;
  }

  @Override
  public final void readFloats(int total, ColumnVectorBase c, int rowId) {
    c.putFloats(rowId, total, buffer, offset - Platform.BYTE_ARRAY_OFFSET);
    offset += 4 * total;
  }

  @Override
  public final void readDoubles(int total, ColumnVectorBase c, int rowId) {
    c.putDoubles(rowId, total, buffer, offset - Platform.BYTE_ARRAY_OFFSET);
    offset += 8 * total;
  }

  @Override
  public final void readBytes(int total, ColumnVectorBase c, int rowId) {
    for (int i = 0; i < total; i++) {
  ...
  @Override
  public final void readBinary(int total, ColumnVectorBase v, int rowId) {
    for (int i = 0; i < total; i++) {
      int len = readInteger();
      int start = offset;
      offset += len;
