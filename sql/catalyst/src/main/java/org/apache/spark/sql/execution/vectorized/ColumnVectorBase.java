 ...
 *
 * To handle nested schemas, ColumnVectorBase has two types: Arrays and Structs. In both cases these
 ...
 *
 * A ColumnVectorBase should be considered immutable once originally created. In other words, it is not
...
public abstract class ColumnVectorBase implements AutoCloseable {
  ...
  public static final class Array extends ArrayData {
    ...
    public final ColumnVectorBase data;
    public int length;
    public int offset;
    ...
    protected Array(ColumnVectorBase data) {
      this.data = data;
    }
  ...
  public void reset() {
    ...
    if (childColumns != null) {
      ...
      for (ColumnVectorBase c: childColumns) {
        c.reset();
      }
    }
    numNulls = 0;
   ...
   */
  public ColumnarBatchBase.Row getStruct(int rowId) {
   ...
   */
  public ColumnarBatchBase.Row getStruct(int rowId, int size) {
    resultStruct.rowId = rowId;
    return resultStruct;
  }
   ...
   */
  public final ColumnVectorBase arrayData() { return childColumns[0]; }

  /**
   * Returns the ordinal's child data column.
   */
  public final ColumnVectorBase getChildColumn(int ordinal) { return childColumns[ordinal]; }
   ...
   */
  protected final ColumnVectorBase[] childColumns;
   ...
   */
  protected final ColumnarBatchBase.Row resultStruct;
   ...
   */
  protected ColumnVectorBase dictionaryIds;
   ...
   */
  public ColumnVectorBase reserveDictionaryIds(int capacity) {
   ...
   */
  public ColumnVectorBase getDictionaryIds() {
    return dictionaryIds;
  }
   ...
   * type.
   */
  protected ColumnVectorBase(int capacity, DataType type, MemoryMode memMode) {
    ...
    if (type instanceof ArrayType || type instanceof BinaryType || type instanceof StringType
      ...
      this.childColumns = new ColumnVectorBase[1];
      this.childColumns[0] = ColumnVectorBase.allocate(childCapacity, childType, memMode);
      this.resultArray = new Array(this.childColumns[0]);
      this.resultStruct = null;
    } else if (type instanceof StructType) {
      StructType st = (StructType)type;
      this.childColumns = new ColumnVectorBase[st.fields().length];
      for (int i = 0; i < childColumns.length; ++i) {
        this.childColumns[i] = ColumnVectorBase.allocate(capacity, st.fields()[i].dataType(), memMode);
      }
      this.resultArray = null;
      this.resultStruct = new ColumnarBatchBase.Row(this.childColumns);
    } else if (type instanceof CalendarIntervalType) {
      // Two columns. Months as int. Microseconds as Long.
      this.childColumns = new ColumnVectorBase[2];
      this.childColumns[0] = ColumnVectorBase.allocate(capacity, DataTypes.IntegerType, memMode);
      this.childColumns[1] = ColumnVectorBase.allocate(capacity, DataTypes.LongType, memMode);
      this.resultArray = null;
      this.resultStruct = new ColumnarBatchBase.Row(this.childColumns);
    } else {
