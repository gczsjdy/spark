 ...
 *
 * The ColumnarBatchBase supports either on heap or offheap modes with (mostly) the identical API.
 *
 * TODO:
...
public class ColumnarBatchBase implements Serializable{
    ...
    private int numRows;
    protected final ColumnVectorBase[] columns;
    ...
    public static ColumnarBatchBase allocate(StructType schema, MemoryMode memMode) {
        return new ColumnarBatchBase(schema, DEFAULT_BATCH_SIZE, memMode);
    }

    public static ColumnarBatchBase allocate(StructType type) {
        return new ColumnarBatchBase(type, DEFAULT_BATCH_SIZE, DEFAULT_MEMORY_MODE);
    }

    public static ColumnarBatchBase allocate(StructType schema, MemoryMode memMode, int maxRows) {
        return new ColumnarBatchBase(schema, maxRows, memMode);
    }
     ...
     */
    public void close() {
        for (ColumnVectorBase c: columns) {
            c.close();
        }
    }
    ...
    public static final class Row extends InternalRow {
        protected int rowId;
        private final ColumnarBatchBase parent;
        private final int fixedLenRowSize;
        private final ColumnVectorBase[] columns;

        // Ctor used if this is a top level row.
        private Row(ColumnarBatchBase parent) {
            this.parent = parent;
        ...
        protected Row(ColumnVectorBase[] columns) {
            this.parent = null;
        ...
        public ColumnVectorBase[] columns() { return columns; }
    ...
    public Iterator<Row> rowIterator() {
        final int maxRows = ColumnarBatchBase.this.numRows();
        ...
        return new Iterator<Row>() {
            ...
            public boolean hasNext() {
                while (rowId < maxRows && ColumnarBatchBase.this.filteredRows[rowId]) {
                    ++rowId;
                }
     ...
     */
    public ColumnVectorBase column(int ordinal) { return columns[ordinal]; }
     ...
     * projections.
     */
    public void setColumn(int ordinal, ColumnVectorBase column) {
    ...
    protected ColumnarBatchBase(StructType schema, int maxRows, MemoryMode memMode) {
        this.schema = schema;
        this.capacity = maxRows;
        this.columns = new ColumnVectorBase[schema.size()];
        this.nullFilteredColumns = new HashSet<>();
        this.filteredRows = new boolean[maxRows];

        for (int i = 0; i < schema.fields().length; ++i) {
            StructField field = schema.fields()[i];
            columns[i] = ColumnVectorBase.allocate(maxRows, field.dataType(), memMode);
        }
